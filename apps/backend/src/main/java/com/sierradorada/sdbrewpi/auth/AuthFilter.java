...
class AuthFilter extends OncePerRequestFilter {
 ...
 @Override protected void doFilterInternal(HttpServletRequest req,HttpServletResponse res,FilterChain chain)throws ServletException,IOException{try{AuthenticatedSession s=service.authenticate(AuthController.bearer(req));if("VIEWER".equals(s.user().role())&&!"GET".equals(req.getMethod())){res.setStatus(403);res.setContentType("application/json");res.setCharacterEncoding("UTF-8");res.getWriter().write("{\"code\":\"FORBIDDEN\",\"message\":\"El rol de solo lectura no permite realizar cambios\"}");return;}req.setAttribute("authUser",s.user());chain.doFilter(req,res);}catch(UnauthorizedException e){res.setStatus(401);res.setContentType("application/json");res.setCharacterEncoding("UTF-8");res.getWriter().write("{\"code\":\"UNAUTHORIZED\",\"message\":\"Debes iniciar sesión para continuar\"}");}}
}
