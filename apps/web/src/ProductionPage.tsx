...
function BatchRecordPanel({ batches, events, busy, onAdd }: { batches: Batch[]; events: Record<string, BatchEvent[]>; busy: string; onAdd: (batchId: string, values: BatchEventInput) => Promise<void> }) {
  ...
  return <section className="batch-record">
    <div className="batch-record-heading"><div><ClipboardList size={19}/><span><b>Batch record digital</b><small>Identidad, asignación y eventos trazables del lote</small></span></div><select aria-label="Lote de la bitácora" value={batchId} onChange={event => setSelectedBatch(event.target.value)}>{batches.map(batch => <option key={batch.id} value={batch.id}>{batch.code} · {batch.tankNameSnapshot ?? batch.status}</option>)}</select></div>
    <div className="batch-record-identity">
      <div><small>LOTE MAESTRO</small><b>{selected.code}</b><span>{selected.productName} · {selected.batchKind === 'TEST' ? 'Prueba técnica' : selected.batchKind === 'PILOT' ? 'Piloto' : 'Comercial'}</span></div>
      <div><small>ESTILO / RECETA</small><b>{selected.recipeName}</b><span>{selected.recipeCode} · versión {selected.recipeVersion}</span></div>
      <div><small>EXPEDIENTE ABIERTO</small><b>{selected.batchRecordOpenedAt ? new Date(selected.batchRecordOpenedAt).toLocaleString('es-CO') : 'Pendiente'}</b><span>Por {selected.releasedBy ?? '—'}</span></div>
      <div><small>FERMENTADOR</small><b>{selected.tankNameSnapshot ?? 'Sin asignar'}</b><span>{selected.tankId ?? '—'} · {selected.fermentationVolumeL?.toFixed(1) ?? '—'} L transferidos</span></div>
      <div><small>SENSOR ASIGNADO</small><b>{selected.pillIdSnapshot ?? 'Sin asignar'}</b><span>{selected.pillSourceSnapshot ?? 'Se captura al transferir'}</span></div>
      <div><small>ASIGNACIÓN</small><b>{selected.fermentationAssignedAt ? new Date(selected.fermentationAssignedAt).toLocaleString('es-CO') : 'Pendiente'}</b><span>Por {selected.fermentationAssignedBy ?? '—'}</span></div>
    </div>
...
export default function ProductionPage() {
  ...
  const fermentingBatches = production?.activeBatches.filter(batch => batch.status === 'FERMENTING') ?? []
  const statusLabel: Record<string, string> = { RELEASED: 'Orden liberada', READY_FOR_FERMENTATION: 'Lista para fermentar', FERMENTING: 'En fermentación' }
  ...
  return <main className="production-page">
    ...
    {!overview || !production ? <div className="loading"><RefreshCw className="spin"/>Cargando producción…</div> : section === 'overview' ? <>
      ...
      <ProductionOrderPanel production={production} onChanged={load}/>
      <section className="active-production"><div className="section-title"><div><span className="line"/><h3>LOTES EN CURSO</h3></div><small>Batch records abiertos</small></div><div className="active-batch-grid">{production.activeBatches.map(batch => <article key={batch.id}><div><small>{batch.code}</small><b>{batch.recipeName}</b></div><span>{batch.volumeL.toFixed(0)} L · {statusLabel[batch.status] ?? batch.status}</span>{batch.status !== 'RELEASED' && <button onClick={() => setSection('fermentation')}>Fermentación <ArrowRight size={13}/></button>}</article>)}</div></section>
      <ProcessMap stages={production.processStages} hasActiveBatch={fermentingBatches.length > 0} onOpenFermentation={() => setSection('fermentation')}/>
    </> : <>
      ...
      <ProfileExecutionPanel batches={fermentingBatches} generatedAt={overview.generatedAt} busy={busy} onAction={(batch, action) => command(batch.id, () => controlProfile(batch.id, action, batch.revision))}/>
      <BatchRecordPanel batches={fermentingBatches} events={batchEvents} busy={busy} onAdd={(batchId, values) => command(`event:${batchId}`, () => addBatchEvent(batchId, values), 'Evento agregado al batch record')}/>
      ...
      <section className="tank-grid">{overview.tanks.map(tank => <TankCard key={`${tank.id}-${tank.setpointC}`} tank={tank} batch={fermentingBatches.find(batch => batch.tankId === tank.id)} history={histories[tank.id]} busy={busy === tank.id} onMode={(t,m) => command(t.id, () => setMode(t,m))} onSetpoint={(t,v) => command(t.id, () => setSetpoint(t,v))}/>)}</section>
